...
$GGUIMAIN = GUICreate ( "APM V" & $GAPMVERSION & " Nickets" , 358 , 680 , 920 , 180 , BitOR ( $GUI_SS_DEFAULT_GUI , $WS_SIZEBOX , $WS_THICKFRAME ) )
...
Func _CHECKFORUPDATE ( )
        ...
        $OHTTP .Open ( "GET" , "https://api.github.com/repos/bisalog365-commits/APM/releases/latest" , False )
...
Func GETBROWSERS ( )
        ...
        For $I = 1 To $ISUNCOUNT
                ...
                If $SSEARCH = - 1 Then
                        ...
                        $SNEEDSSORT = True
                Else
                        ; Re-resolve profile label if it still shows a raw user_id.
                        ; Happens when profiles were already open before APM started
                        ; and the AdsPower cache was empty on first detection.
                        Local $SCURPROFILE = $GBROWSERS [ $SSEARCH ] [ 2 ]
                        If StringRegExp ( $SCURPROFILE , "^[a-z][a-z0-9]+$" ) Then
                                Local $SRETRYID = _GETADSPOWERUSERID ( $SHANDLE )
                                If $SRETRYID <> "" Then
                                        Local $SRETRYCUSTOM = _GETADSPOWERCUSTOMNO ( $SRETRYID )
                                        If $SRETRYCUSTOM <> "" And $SRETRYCUSTOM <> $SCURPROFILE Then
                                                $II = _GUICTRLLISTVIEW_FINDINTEXT ( $LISTVIEW1 , $SHANDLE )
                                                _GUICTRLLISTVIEW_SETITEMTEXT ( $LISTVIEW1 , $II , $SRETRYCUSTOM , 0 )
                                                $GBROWSERS [ $SSEARCH ] [ 2 ] = $SRETRYCUSTOM
                                                $SNEEDSSORT = True
                                        EndIf
                                EndIf
                        EndIf
